 ...
 * 
 */
package android.library.asyncimageloader;

import java.util.HashMap;
import java.util.HashSet;

import android.graphics.Bitmap;
import android.library.asyncimageloader.utils.BitmapUtils;
	...
	public void postRequest(final ImageView view, final String url, float round){
		...
		if (bitmap == null) {
			...
			if (schedule.round > 0) {//圆角处理
				bitmap = BitmapUtils.drawRoundBitmap(bitmap, schedule.round);
			}
		...
		if (loadingRequest.containsKey(url)) {
			...
			for (ImageRequestSchedule imageRequestSchedule : scheduleSet) {
				if (imageRequestSchedule.round > 0) { //圆角处理
					if (bitmap != null) {
						bitmap = BitmapUtils.drawRoundBitmap(bitmap, imageRequestSchedule.round);
					}
				}
